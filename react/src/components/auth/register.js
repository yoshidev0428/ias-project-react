    ...
    authApi
      ...
      .then(response => {
        if (response.status === 201) {
          ...
          store.dispatch({type:"auth_setAuthSecrets", payload:{
    ...
    <div className='login-container'>
      <Container className='inner-container model rules auto-complete label-position'>
        ...
        <Form onSubmit={callSubmit}>
          ...
          <Button variant="primary" type="submit" block style={{width:'100%', height: '50px', color: 'black', marginBottom: '3px', background: 'lightblue', borderRadius: '2px'}}>
            Register
          </Button>
