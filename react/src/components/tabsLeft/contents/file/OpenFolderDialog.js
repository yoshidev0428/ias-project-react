...
const OpenFolderDialogForUpload = (props) => {
  const experiment_name =
    'experiement_' + new Date().toISOString().replaceAll(':', '-');
  ...
  const [_photo123, setphoto123] = useState(null);
  const [_image, setFiles] = useState([]);
  ...
  let registerData = [];
  if (selectedFolder != '') {
    const data = props.experiments.filter(
      (experiment) => experiment.label == selectedFolder,
    )[0];
  ...
  const onFolderChangeCapture = async (e) => {
    if (e.target.files.length > 0) {
      ...
      if (newAcceptedFiles.length > 0) {
        //************************************************************************** */
        let resUpload = await api_tiles.uploadImages(
          newAcceptedFiles,
          uploadFolderName,
        );
        if (resUpload.status == 200) setsuccessDialog(true);
        setFiles([]);
        setphoto123(null);
        getTree();
      }
...
OpenFolderDialogForUpload.propTypes = {
